...
export async function getTrainServices(
	...
	const url = `/api/dept/${crs}/${numRows}/${filterToc}/${date}`;
	const response = await fetch(url);
	const data: definitions['StationBoard'] = await response.json();
	const trainServices: [string, definitions['ServiceItem']][] = data.trainServices!.map((t: definitions['ServiceItem']) => [t.rid, t]);
