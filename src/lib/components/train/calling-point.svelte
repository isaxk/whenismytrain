<script lang="ts">
	...
	import { Check, ChevronDown, ChevronRight, Train } from 'lucide-svelte';
	...
	let { i, operator, location, length } = $props();
...
{/if}
<Accordion.Item
	class="even:bg-muted odd:bg-card group border-border relative transition-all duration-200 first:border-none"
...
>
	<div bind:clientHeight class="flex gap-4 pr-0 pl-5">
		...
		<div class={['min-w-5']}>
			<div class="absolute -top-1 -bottom-1 flex flex-col justify-center pl-1">
				{#if i !== 0}
					<div
						class={['bg-accent h-8 w-2', location.status !== Status.DEPARTED && 'saturate-[80%]']}
				...
				<div
					class={[
						...
						location.status !== Status.DEPARTED && 'saturate-[80%]'
					]}
					style:background={operatorList[operator].bg}
				></div>
				{#if i !== length - 1}
				...
				{/if}
				{#if location.status === Status.DEPARTED && location.progress !== 1}
				...
				{:else if location.status === Status.ARRIVED}
					<div class="absolute top-6 -left-1 flex w-2">
						<div
							in:receive={{ key: 'pos' }}
							out:send={{ key: 'pos' }}
							class="flex h-6 min-w-6 items-center justify-center rounded-full border-2 bg-white"
				...
				>{#snippet child({ props, open })}
					{#if open}
						<div
							...
							{...props}
						>
							<div class="text-foreground-muted text-sm font-light italic">
								{#if location.isCancelled}
									Train has been cancelled at this location
								{:else if location.status === Status.DEPARTED}
									Train has departed this location
								{:else if location.status === Status.ARRIVED}
									Train has arrived at this location
								{:else if location.times.scheduled.arrival}
									Train has not yet arrived at this location
								{:else}
									Train has not yet departed at this location
								{/if}
							</div>
